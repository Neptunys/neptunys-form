...
import { applyLogicToFields } from '@neptunysform-inc/answer-utils'
...
export const FormRenderer: FC<FormRendererProps> = ({
  ...
  useEffect(() => {
    setAndroid(window.neptunysform.device.android)
      ...
      <Tooltip.Provider delayDuration={100}>
        <div
          className={clsx(
            'neptunysform-root',
            {
              'neptunysform-root-embedded': isEmbedded,
              'neptunysform-root-open': state.isSidebarOpen,
              'neptunysform-root-android': isAndroid
            },
            className
          )}
        >
          <div
            className={clsx('neptunysform-wrapper', {
              'neptunysform-is-welcome': !state.isStarted && state.welcomeField
            })}
          >
            <Blocks />
          </div>
