...
import type { FormField, FormSettings } from '@neptunysform-inc/shared-types-enums'
import { FieldKindEnum, NumberPrice } from '@neptunysform-inc/shared-types-enums'
...
import { applyLogicToFields, validateFields } from '@neptunysform-inc/answer-utils'
...
export const Form: FC<FormProps> = ({
  ...
  useEnterKey(`neptunysform-${state.instanceId}-${field.id}`, (event: KeyboardEvent) => {
  ...
  return (
    <RCForm
      className={clsx('neptunysform-form', 'neptunysform-form-mobile-docked', {
        'neptunysform-form-last': isLastBlock
      })}
      ...
      {...formProps}
    >
      <div className="neptunysform-form-content">{children}</div>
      ...
      {isLastBlock || state.isScrollNextDisabled ? (
        <>
          {submitError && (
            <div className="neptunysform-validation-wrapper">
              <div className="neptunysform-validation-error">{submitError}</div>
            </div>
          )}
          <div className="neptunysform-form-actions">
            {showBackButton && (
              <button
                className="neptunysform-back-button"
                type="button"
                aria-label={t('paginationPrevious')}
                onClick={handlePrevious}
              >
                <IconChevronLeft className="neptunysform-back-button-icon" />
                <span className="neptunysform-back-button-text">{t('paginationPrevious')}</span>
              </button>
            )}
        ...
        <div className="neptunysform-form-actions">
          ...
          {isSkippable && (
            <button className="neptunysform-skip-button" type="button" onClick={handleSkip}>
              {t('Skip')}
            </button>
          )}
        </div>
