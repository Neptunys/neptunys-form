...
import { getDateFormat } from '@neptunysform-inc/answer-utils'
import { helper, date as vdate } from '@neptunysform-inc/utils'
...
const DateItem: FC<DateItemProps> = ({ format, value: rawValue = '', onChange, onError }) => {
  ...
  return (
    <div className={`neptunysform-date-input neptunysform-date-item-${id}`}>
      <label htmlFor={`neptunysform-date-${id}`} className="neptunysform-date-label">
        {t(label)}
      </label>
      <Input id={`neptunysform-date-${id}`} value={value} placeholder={format} onChange={handleChange} />
    </div>
  )
}
...
export const DateInput: FC<DateInputProps> = ({
  ...
  return (
    <div className="neptunysform-date-root">
      ...
      <div className="neptunysform-date-divider">{divider}</div>
