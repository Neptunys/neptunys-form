...
const Item: FC<ItemProps> = ({
  ...
  return (
    <div
      id={`neptunysform-country-${country.value}`}
      className={clsx('neptunysform-radio', {
        'neptunysform-radio-selected': isSelected,
        'neptunysform-radio-highlighted': isHighlighted
      })}
    ...
    >
      <div className="neptunysform-radio-container">
        <div className="neptunysform-radio-content">
          ...
          {enableCallingCode && (
            <span className="neptunysform-radio-calling-code mr-2">+{country.callingCode}</span>
          )}
          <span className="neptunysform-radio-label">{t(country.label)}</span>
        </div>
        {isSelected && (
...
export const CountrySelect: FC<CountrySelectProps> = ({
  ...
  const memoOverlay = useMemo(() => {
    return (
      <div
        className={clsx('neptunysform-select-popup', popupClassName)}
  ...
  function scrollIntoView(value: string) {
    setTimeout(() => {
      document.getElementById(`neptunysform-country-${value}`)?.scrollIntoView({ block: 'nearest' })
    }, 0)
  }
  ...
  useEffect(() => {
    if (isOpen) {
      document.body.classList.add('neptunysform-dropdown-open')
      setTriggerStyle(ref?.getBoundingClientRect())
    } else {
      document.body.classList.remove('neptunysform-dropdown-open')
    }
    ...
    <>
      <div
        ...
        className={clsx(
          'neptunysform-select',
          {
            'neptunysform-select-open': isOpen,
      ...
      >
        <div className="neptunysform-select-container">
          <div className="neptunysform-select-value">
            {selected && <FlagIcon className="mr-2" countryCode={selected.value} />}
            {enableLabel && (
              // @ts-ignore
              <span className="neptunysform-select-label" data-placeholder={placeholder}>
          ...
          {selected && allowClear && (
            <Tooltip className="neptunysform-select-clear" ariaLabel={t('Clear')}>
          ...
          <div className="neptunysform-select-arrow-icon">
            {isOpen ? <IconChevronUp /> : <IconChevronDown />}
          </div>
        </div>
        <div className="neptunysform-group-highlight" />
      </div>
