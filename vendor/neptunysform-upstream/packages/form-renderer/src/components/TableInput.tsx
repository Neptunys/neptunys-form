import type { Column, InputTableValue } from '@neptunysform-inc/shared-types-enums'
...
export const TableInput: FC<TableInputProps> = ({
  ...
  return (
    <div className="neptunysform-table-root" {...restProps}>
      <table>
        <thead className="neptunysform-table-header">
          <tr>
            {columns?.map(column => (
        ...
        <tbody>
          {rows.map((_, rowIdx) => (
            <tr key={rowIdx} className="neptunysform-table-row">
              {columns?.map(column => (
                <InputField
                  key={column.id}
