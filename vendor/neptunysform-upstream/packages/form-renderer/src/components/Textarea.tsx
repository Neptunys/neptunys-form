...
export const Textarea: FC<TextareaProps> = ({ value: rawValue, onChange, ...restProps }) => {
  ...
  function handleKeyDown(e: KeyboardEvent<HTMLDivElement>) {
    if (e.key === 'Enter') {
      ...
      if (e.shiftKey || window.neptunysform.device.mobile) {
  ...
  return (
    <>
      <div
        ...
        className="neptunysform-textarea"
      ...
      />
      {isTipShow && (
        <div className="neptunysform-textarea-tip">{t('Hit Shift ⇧ + Enter ↵ for new line')}</div>
      )}
    </>
  )
}
...
export const AutoResizeTextarea: FC<AutoResizeTextareaProps> = ({
  ...
  return (
    <textarea
      ref={inputRef}
      className={clsx('neptunysform-autoresize-textarea', {
        'neptunysform-autoresize-invalid': !window.CSS.supports('field-sizing', 'content')
      })}
