...
const MODAL_TEMPLATE = `
  <div id="{containerId}" class="neptunysform__modal neptunysform__modal-{size}">
    ...
    <button type="button" class="neptunysform__close-button" onclick="NeptunysForm.closeModal('{formId}')">
      ${IconClose}
    </button>
  </div>
`
...
export class Modal<T extends ModalSettings> extends Standard<T> {
  ...
  override render() {
    const $button = this.$container.find('.neptunysform__trigger-button')
    ...
    if ($button.exists()) {
      if (this.settings.triggerBackground) {
        ...
        if (colorIsDark(this.settings.triggerBackground)) {
          $button.addClass('neptunysform__trigger-button-dark')
        }
      }
  ...
  protected handleEvents() {
    ...
    if (this.settings.hideAfterSubmit) {
      window.onmessage = ({ data }: MessageEvent) => {
        if (!isPlainObject(data) || data.source !== 'NEPTUNYSFORM') {
          return
        }
