...
const POPUP_TEMPLATE = `
  <div id="{containerId}" class="neptunysform__popup neptunysform__popup-{position}">
    <button class="neptunysform__popup-button {buttonClass}" style="{buttonStyle}" onclick="NeptunysForm.togglePopup('{formId}')">
      {icon}
    </button>
  </div>
`
...
const IFRAME_TEMPLATE = `
  <div class="neptunysform__iframe-container" style="{style}">
...
export class Popup<T extends PopupSettings> extends Modal<T> {
  override render() {
    ...
    if (this.settings.triggerBackground) {
      ...
      if (colorIsDark(this.settings.triggerBackground)) {
        data.buttonClass = 'neptunysform__popup-button-dark'
      }
    }
  ...
  public open() {
    ...
    const $frame = $el.find('.neptunysform__iframe-container')
    ...
    setTimeout(() => {
      $el.find('iframe').get(0).onload = () => {
        $el.find('.neptunysform__loading-container').remove()
      }
    }, 0)
  ...
  public close() {
    $(`#${this.containerId} .neptunysform__iframe-container`).remove()
  ...
  private updateButtonIcon() {
    const button = $(`#${this.containerId} .neptunysform__popup-button`)
