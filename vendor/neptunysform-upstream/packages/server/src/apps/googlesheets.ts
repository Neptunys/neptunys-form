...
function groupTestPayloadsByDestination(config: GoogleSheetsConfig, payloads: LeadCapturePayload[]) {
  ...
  for (const payload of payloads) {
    ...
    const row = applyIncludedAnswerColumns(
      ...
      payload,
      config.includedAnswerFieldsByForm?.[payload.formId]
    )
...
async function writeLeadRow(
  ...
  const row = applyIncludedAnswerColumns(
    ...
    enrichedPayload,
    config.includedAnswerFieldsByForm?.[enrichedPayload.formId]
  )
...
export default {
  ...
  settings: [
    ...
    },
    {
      ...
      placeholder: 'neptunysform-sync@project-id.iam.gserviceaccount.com',
      required: true
    },
    {
      type: 'textarea',
