...
import { cn, getDecoratedURL, isRenderableImageSource } from '@/utils'
import { helper, removeObjectNil } from '@neptunysform-inc/utils'
...
const Background: FC<BackgroundProps> = ({
  ...
  const isImage = useMemo(() => isRenderableImageSource(src), [src])
...
const ImageComponent: FC<ImageProps> = ({
  ...
  const src = useMemo(() => {
    if (!isRenderableImageSource(rawSrc as string)) {
      return
    }

    if (helper.isURL(rawSrc as string) && (helper.isNumber(width) || helper.isNumber(height))) {
      return getDecoratedURL(
        '/api/image',
        removeObjectNil({
          url: rawSrc as string,
