import { CHOICE_FIELD_KINDS, FieldKindEnum, RATING_FIELD_KINDS } from '@neptunysform-inc/shared-types-enums'
import { FC, useMemo } from 'react'
import { useTranslation } from 'react-i18next'

import { helper, toFixed } from '@neptunysform-inc/utils'
...
const Choices: FC<ChoicesProps> = ({ chooses }) => {
  ...
  return (
    <div className="neptunysform-report-chart">
      {chooses.map((row, index) => {
        const percent = `${toFixed((row.count * 100) / total)}%`

        return (
          <div key={index} className="neptunysform-report-chart-item">
            <div
              className="neptunysform-report-chart-background"
              style={{
                width: percent
              }}
            />
            <div className="neptunysform-report-chart-content">
              <span className="neptunysform-report-chart-percent">
                {row.label} · {percent}
              </span>
              <span className="neptunysform-report-chart-count">
...
const CompletionCoverage: FC<CompletionCoverageProps> = ({ count, total }) => {
  ...
  return (
    <div className="neptunysform-report-chart">
      <div className="neptunysform-report-chart-item">
        <div
          className="neptunysform-report-chart-background"
          style={{
            width: percent
...
const FormReportItem: FC<FormReportItemProps> = ({ index, response, isHideFieldEnabled }) => {
  ...
  return (
    <li className="neptunysform-report-item">
      <div className="flex gap-4">
        <div className="neptunysform-report-question flex-1">
          {index}. {response.title}
        </div>
      </div>
      <div className="neptunysform-report-meta">
        {isRating
      ...
      {!isHided && <div className="neptunysform-report-content">{children}</div>}
    </li>
  )
}
