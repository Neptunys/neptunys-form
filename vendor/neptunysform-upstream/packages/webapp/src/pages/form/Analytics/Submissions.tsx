import { Column, FieldKindEnum } from '@neptunysform-inc/shared-types-enums'
      ...
      <thead>
        <tr>
          {columns.map(c => (
            <th
              key={c.id}
              className="neptunysform-report-border text-secondary border-b py-2 text-sm/6 font-medium"
            >
              {c.label}
            </th>
          ))}
          <th className="neptunysform-report-border border-b"></th>
        </tr>
      </thead>

      <tbody className="neptunysform-report-divide divide-y">
        {answers.map((row: any, index: number) => (
          <tr key={index}>
            {columns.map(c => (
              <td key={c.id} className="neptunysform-report-input-value">
                {row.value[c.id]}
              </td>
            ))}
            <td className="neptunysform-report-input-datetime">
...
const SubmissionItem: FC<SubmissionItemProps> = ({ answers = [] }) => {
  ...
  return (
    <div className="neptunysform-report-divide divide-y">
      {answers.map(row => (
        <div className="neptunysform-report-answer" key={row.submissionId}>
          <div className="neptunysform-report-value">
            <AnswerValue answer={row} />
          </div>
          <div className="neptunysform-report-datetime">{timeFromNow(row.endAt, i18n.language)}</div>
        </div>
      ))}
    </div>
  )
...
export default function FormReportSubmissions({ response }: any) {
  ...
  return (
    <div>
      ...
      {total > 10 && (
        <Pagination
          className="neptunysform-report-pagination"
          total={total}
          page={page}
          pageSize={10}
          buttonProps={{
