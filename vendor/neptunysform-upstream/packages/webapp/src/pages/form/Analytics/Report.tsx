...
export const ReportList: FC<ReportListProps> = ({ isHideFieldEnabled }) => {
  ...
  return (
    ...
    >
      {isHideFieldEnabled && (
        <>
          <h2 className="neptunysform-report-heading">{form?.name}</h2>
          <p className="neptunysform-report-subheading">
            {t('form.customReport.metadata', { count: form?.submissionCount || 0 })}
          </p>
        </>
      )}

      <ol className="neptunysform-report-items mt-4 space-y-8">
...
export default function FormAnalyticsReport() {
  ...
  return (
    <>
      ...
      <div className="p-6">
        <div className="neptunysform-report">
          <ReportList />
        </div>
      </div>
    </>
