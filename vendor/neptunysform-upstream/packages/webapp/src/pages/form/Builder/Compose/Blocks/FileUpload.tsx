...
export const FileUpload: FC<BlockProps> = ({ field, locale, ...restProps }) => {
  ...
  return (
    <Block className="neptunysform-file-upload" field={field} locale={locale} {...restProps}>
      <div className="neptunysform-file-uploader">
        <div className="neptunysform-upload-wrapper">
          <IconUpload className="neptunysform-upload-icon non-scaling-stroke" />
          <div className="mt-8">{t('Upload a file or drag and drop', { lng: locale })}</div>
          <div className="neptunysform-upload-size-limit">{t('Size limit', { lng: locale })}: 10MB</div>
        </div>
      </div>
