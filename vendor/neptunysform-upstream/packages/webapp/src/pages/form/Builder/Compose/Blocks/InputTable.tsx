import { Button } from '@neptunysform-inc/form-renderer'
import type { Choice } from '@neptunysform-inc/shared-types-enums'
import { IconChevronRight, IconX } from '@tabler/icons-react'
import type { FC } from 'react'
import { startTransition, useCallback, useState } from 'react'
import { useTranslation } from 'react-i18next'

import { clone, nanoid } from '@neptunysform-inc/utils'
...
const Thead: FC<TheadProps> = ({ index, column, deletable, onChange, onRemove, ...restProps }) => {
  ...
  return (
    <th>
      <div className="neptunysform-input-table-thead" {...restProps}>
...
export const InputTable: FC<BlockProps> = ({ field, locale, ...restProps }) => {
  ...
  return (
    <Block className="neptunysform-input-table" field={field} locale={locale} {...restProps}>
      <div className="mb-2 flex items-center justify-end">
        <Button.Link className="neptunysform-add-column" onClick={handleAddColumnCallback}>
          {t('form.builder.compose.addColumn')}
        </Button.Link>
      </div>
      <div className="neptunysform-table-scrollable">
        <table>
          <thead>
            <tr className="neptunysform-input-table-header">
          ...
          <tbody>
            {Array.from({ length: 5 }).map((_, i) => (
              <tr key={i} className="neptunysform-input-table-row">
