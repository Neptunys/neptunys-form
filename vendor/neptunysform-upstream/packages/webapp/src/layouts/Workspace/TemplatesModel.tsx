import { FormRenderer, insertWebFont } from '@neptunysform-inc/form-renderer'
...
import { cn, scrollIntoViewIfNeeded, useParam } from '@/utils'
import { slugify } from '@neptunysform-inc/utils'
...
const TemplatePreview: FC<TemplatePreviewProps> = ({ template: rawTemplate, onBack }) => {
  ...
  const { loading, run } = useRequest(
    async () => {
      ...
      window.location.assign(`/workspace/${workspaceId}/project/${projectId}/form/${formId}/create`)
    },
    {
      ...
      onError(err: any) {
        toast({
          ...
          message: err?.message || t('components.error.message')
        })
      }
    }
  )
  ...
  const { loading: importLoading, run: importForm } = useRequest(
    async (formJson: string) => {
      try {
        ...
        closeModal('CreateFormModal')
        window.location.assign(`/workspace/${workspaceId}/project/${projectId}/form/${result}/create`)
      } catch (error) {
        toast({
          title: t('components.error.title'),
          message:
