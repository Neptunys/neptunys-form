...
This repository now uses NeptunysForm as the live form platform.

The active application source lives in `vendor/neptunysform-upstream` and is tracked by this root repository.
...
- Active app: `vendor/neptunysform-upstream`
- Local helper scripts: `scripts/start-neptunysform-native.ps1` and `scripts/stop-neptunysform-native.ps1`
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\start-neptunysform-native.ps1
```
...
```powershell
powershell -ExecutionPolicy Bypass -File .\scripts\stop-neptunysform-native.ps1
```

## What Was Wired

The NeptunysForm integration in this repo includes:
...
For full live usage, use a host that runs the NeptunysForm server container and keep the frontend and backend on the same origin where possible.

Recommended free-ish stack:

1. Backend app: Render web service using `vendor/neptunysform-upstream/Dockerfile`
2. Database: MongoDB Atlas free tier
3. Redis/Bull: Upstash Redis free tier
4. Frontend: either
   - same backend host via the NeptunysForm server container, or
...
- Supabase can be used for other project data, but it does not replace the NeptunysForm NestJS backend.
- Vercel alone is not enough for full live usage of NeptunysForm because auth, uploads, GraphQL, and static uploaded assets depend on the backend.

## Git And Deploy Notes

- This root repository now tracks the NeptunysForm source directly under `vendor/neptunysform-upstream`.
- The nested upstream `.git` metadata was moved into `.neptunysform-local/repo-backups` so root git can own the app files.
- If you deploy the frontend on Vercel, point the project at the NeptunysForm webapp folder or update the root Vercel project to build from that source.
